...
atlas_android_dns_body() {
  ...
  [ "$n" -gt 0 ] || {
    # 3) Global DnsAddresses residual
    if [ -n "$dump" ]; then
  ...
  [ "$n" -gt 0 ] || {
    # 4) Legacy props (often empty on modern Android)
    for p in net.dns1 net.dns2 net.dns3 net.dns4 dhcp.wlan0.dns1 dhcp.wlan0.dns2; do
      v=`getprop "$p" 2>/dev/null`
      [ -n "$v" ] && add "$v"
    done
  }
  if [ "$n" -eq 0 ]; then
    # 5) If a tun is up we still refuse public DNS (would ignore VPN).
    if ip -o link show 2>/dev/null | grep -q ' tun[0-9][0-9]*:'; then
      return 1
    fi
    # 6) Last resort only when no Android DNS and no tun.
    echo "nameserver 8.8.8.8"
    echo "nameserver 1.1.1.1"
  fi
  # Search domains (ts.net / lan) for MagicDNS + LAN short names
  if [ -n "$dump" ]; then
...
if [ "$want_hybrid" = "1" ]; then
  ...
  # Product path: /system/bin/atlas-enter only (never app-private ELF → exit 126).
  if [ -n "$ENTER" ]; then
    case "$ENTER" in
      /data/data/*|/data/user/*)
        if [ -x /system/bin/atlas-enter ]; then
          ENTER=/system/bin/atlas-enter
        fi
        ;;
    esac
    if ! hybrid_ready; then
      # Ask init watch; short wait so cold boot enterd/bind can land (reboot 126 residual)
