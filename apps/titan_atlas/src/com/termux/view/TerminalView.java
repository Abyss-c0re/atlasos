...
public final class TerminalView extends View {
    ...
    public TerminalView(Context context, AttributeSet attributes) { // NO_UCD (unused code)
        ...
        mGestureRecognizer = new GestureAndScaleRecognizer(context, new GestureAndScaleRecognizer.Listener() {
            ...
            public boolean onScroll(MotionEvent e, float distanceX, float distanceY) {
                if (mEmulator == null) return true;
                // Always scroll the terminal for finger/pad drag.
                // Mouse-move CSI is disabled (SGR thrash); taking that branch made scroll a no-op
                // when the device reported SOURCE_MOUSE (Titan pad/mouse mode).
    ...
    public InputConnection onCreateInputConnection(EditorInfo outAttrs) {
        ...
        // NO_EXTRACT / NO action: IME must not steal Enter after Ctrl-C “press enter”.
        ...
        return new BaseInputConnection(this, true) {
            ...
            public boolean commitText(CharSequence text, int newCursorPosition) {
                ...
                // Drop lone DEL/BS that some IMEs inject as text (shows as garbage in TUIs)
                if (content != null && content.length() == 1) {
                    char c = content.charAt(0);
                    if (c == 0x7f || c == 0x08) {
                        if (mTermSession != null) mTermSession.write("\u007f");
                        content.clear();
                        return true;
                    }
                }
            ...
            public boolean deleteSurroundingText(int leftLength, int rightLength) {
                ...
                KeyEvent up = new KeyEvent(KeyEvent.ACTION_UP, KeyEvent.KEYCODE_DEL);
                for (int i = 0; i < n; i++) {
                    sendKeyEvent(down);
                    sendKeyEvent(up);
                }
            ...
            public boolean sendKeyEvent(KeyEvent event) {
                ...
                // Soft IME keys (Del/Enter/arrows) → same onKeyDown as hardware.
    ...
    void doScroll(MotionEvent event, int rowsDown) {
        if (mEmulator == null || rowsDown == 0) return;
        boolean up = rowsDown < 0;
        int amount = Math.abs(rowsDown);
        for (int i = 0; i < amount; i++) {
            if (mEmulator.isAlternateBufferActive()) {
                // Full-screen TUI (Grok): no host transcript u2014 drive the app.
                if (mEmulator.isMouseTrackingActive()) {
                    int button = up ? TerminalEmulator.MOUSE_WHEELUP_BUTTON
                            : TerminalEmulator.MOUSE_WHEELDOWN_BUTTON;
                    sendMouseEventCode(event, button, true);
                } else {
                    // Page is more useful than single arrows for chat/list scroll.
                    handleKeyCode(up ? KeyEvent.KEYCODE_PAGE_UP : KeyEvent.KEYCODE_PAGE_DOWN, 0);
                }
            } else {
                int max = mEmulator.getScreen().getActiveTranscriptRows();
                mTopRow = Math.min(0, Math.max(-max, mTopRow + (up ? -1 : 1)));
    ...
    public boolean onKeyPreIme(int keyCode, KeyEvent event) {
        ...
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_PAGE_DOWN:
                ...
                break;
            default:
                // Ctrl+letter while sticky ctrl: also pre-IME
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        // Titan sticky-Shift kill (T-013): EventHub can leave META_SHIFT / CAPS_LOCK
