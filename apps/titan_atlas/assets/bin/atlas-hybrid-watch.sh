...
kill_all_enterd() {
  for p in `pgrep -x atlas-enterd 2>/dev/null`; do
    kill "$p" 2>/dev/null || true
  done
  # tip binary may show as atlas-enterd-tip in comm — also match full path
  for p in `pgrep -f atlas-enterd 2>/dev/null`; do
    kill "$p" 2>/dev/null || true
  done
  sleep 0.2
}
...
enterd_live() {
  # tip ELF may show as atlas-enterd-tip; system as atlas-enterd
  if pidof atlas-enterd >/dev/null 2>&1; then return 0; fi
  if pidof atlas-enterd-tip >/dev/null 2>&1; then return 0; fi
  return 1
}
...
ensure_enterd() {
  ...
  # Prefer tip when present: kill system enterd orphans that race after tip start
  if [ -x "$ENTERD_TIP" ] && enterd_live; then
    for p in `pidof atlas-enterd 2>/dev/null`; do
      ...
      case "$exe" in
        ...
        *)
          ...
          log "pruned system enterd pid=$p (tip preferred)"
          ;;
      esac
    done
    # tip still live?
