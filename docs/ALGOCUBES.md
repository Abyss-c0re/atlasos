...
| A6 bind 1 or 2 or both | OEM mims_support=2: bind present trays **once at boot**. UICC must not set-ims-service. Never ims disable | setup |
...
| A16 no tray pin | bind = every present SIM any slot; never persist 1\|2 | setup+Controls |
| A17 no UICC-off | setUiccApplicationsEnabled(false) deletes the sub; OEM power-off keeps it listed; undelete Settings UICC-off | SimCards |
